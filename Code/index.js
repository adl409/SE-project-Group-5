...
app.post('/login_init', async function(req,res) {
    ...
    try
    {
        ...
        if (result) {
            ...
            var query = mysql.format("SELECT * FROM SELab.users WHERE username = ?", [username]);
            con.query(query,  async function (err, result) {
                ...
                else if (result[0].type_flag == 2)
                {
                    ...
                    con.query("SELECT * FROM SELab.users", function(err, results) {
                        if (err) throw err;
...
app.post('/create_account_init', async function(req,res) {
    ...
    try
    {
        ...
        var query = mysql.format(`SELECT * FROM SELab.users WHERE username = ? AND password = ?`,[username, password])
